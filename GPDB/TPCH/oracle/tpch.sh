...
function benchmark_run() {
	mkdir -p "$RESULTS"
	# store the settings
	psql postgres -c "select name,setting from pg_settings" > $RESULTS/settings.log 2> $RESULTS/settings.err
	print_log "preparing TPC-H database"
	# create database, populate it with data and set up foreign keys
	#psql -h localhost $DBNAME < dss/tpch-create.sql > $RESULTS/create.log 2> $RESULTS/create.err
	print_log "  loading data"
	psql $DBNAME < dss/tpch-load.sql > $RESULTS/load.log 2> $RESULTS/load.err
	print_log "  creating primary keys"
	psql $DBNAME < dss/tpch-pkeys.sql > $RESULTS/pkeys.log 2> $RESULTS/pkeys.err
	print_log "  creating foreign keys"
	psql $DBNAME < dss/tpch-alter.sql > $RESULTS/alter.log 2> $RESULTS/alter.err
	print_log "  creating indexes"
	psql $DBNAME < dss/tpch-index.sql > $RESULTS/index.log 2> $RESULTS/index.err
	print_log "  analyzing"
...
function benchmark_dss() {
	...
	do
		q="dss/sql/$n.sql"
		qe="dss/sql/$n.explain.sql"

		if [ -f "$q" ]; then
			print_log "  running query $n"
			echo "======= query $n =======" >> $RESULTS/data.log 2>&1;
			# run explain
			psql $DBNAME < $qe > $RESULTS/explain/$n 2>> $RESULTS/explain.err

			vmstat -s > $RESULTS/vmstat-s/before-$n.log 2>&1
			vmstat -d > $RESULTS/vmstat-d/before-$n.log 2>&1

			# run the query on background
			/usr/bin/time -a -f "$n = %e" -o $RESULTS/results.log psql $DBNAME < $q > $RESULTS/results/$n 2> $RESULTS/errors/$n &
			...
			do
				...
				if [ -d "/proc/$!" ]; then
					...
					if [ $i -eq $DSS_TIMEOUT ]; then
						...
						psql postgres -c "SELECT pg_terminate_backend(procpid) FROM pg_stat_activity WHERE datname = 'tpch'" >> $RESULTS/queries.err 2>&1;
						# time to do a cleanup
						sleep 10;
						# just check how many backends are there (should be 0)
						psql postgres -c "SELECT COUNT(*) AS tpch_backends FROM pg_stat_activity WHERE datname = 'tpch'" >> $RESULTS/queries.err 2>&1;
					else
